...
In total, there were `r nrow(bl_data)` participants recruited and `r filter(bl_data, age_at_onset != "NA") |> nrow()`  participants developed MCI. The average baseline age was `r bl_data |> pull(current_age) |>  mean() |> round(2)` years old. Within the total the study participant group, there are  `r filter(bl_data, sex == "female" & apoe4 == "carrier") |> nrow()` women who are APOE4 carriers. 
...
In attempting to check the participants who only appeared in the baseline of amyloid datasets, I found a resource online that utilized the `comparedf()` function within the `"arsenal"` package to be able to run a detailed summary of differences between two datasets. I installed the package and compared the  `bl_data` and `amy_data` data sets by ID numbers using the code below. 
