...
When all three data sets are joined together in the `trash_tidy` dataframe, there are a total of `r nrow(trash_tidy)` observations and `r ncol(trash_tidy)` variables. From years `r mr_trash |> pull(year) |> min()` to `r mr_trash |> pull(year) |> max()`, Mr. Trash Wheel collected a total of `r filter(trash_tidy, name == "mr_trash_wheel") |> pull(weight_tons) |> sum() |> round(2)` tons of trash. Gwynnda Trash Wheel collected a total of `r filter(trash_tidy, name == "gwynnda_trash_wheel", month == "July", year == "2021") |> pull(cigarette_butts) |> sum()` cigarette butts in July of 2021. 
